...
def draw_group_song_rank(
    ...
    *,
    ...
    draw.rounded_rectangle((230, 104, 310, 142), radius=16, fill=type_color)
    type_font = _font(TBFONT, 20)
    type_bbox = draw.textbbox((0, 0), music.type, font=type_font)
    draw.text(((230 + 310 - (type_bbox[2] - type_bbox[0])) / 2, 111), music.type, font=type_font, fill="white")

    diff_colors = ["#71d94b", "#f4bb21", "#ff7180", "#a958df", "#ca9af1"]
    diff_text = f"{diffs[level_index]}  {music.ds[level_index]}"
    diff_bbox = draw.textbbox((0, 0), diff_text, font=_font(SIYUAN, 20))
    diff_right = 340 + diff_bbox[2] - diff_bbox[0] + 36
    draw.rounded_rectangle((328, 104, diff_right, 142), radius=16, fill=diff_colors[level_index])
    draw.text((346, 110), diff_text, font=_font(SIYUAN, 20), fill="white")
    ...
    draw.rounded_rectangle((205, footer_top + 67, 995, footer_top + 112), radius=20, fill=(100, 108, 123, 190))
    credit_font = _font(SIYUAN, 19)
    credit_bbox = draw.textbbox((0, 0), credit, font=credit_font)
    draw.text(((1200 - (credit_bbox[2] - credit_bbox[0])) / 2, footer_top + 78), credit, font=credit_font, fill="white")
    return canvas.convert("RGB")
