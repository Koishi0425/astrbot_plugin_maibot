...
def _progress_rows(count: int, columns: int) -> int:
    return max(4, count // columns + (0 if count % columns == 0 else 1))
...
class DrawScore(ScoreBaseImage):
    ...
    def __init__(self, image: Image.Image = None) -> None:
        ...
        self._im.alpha_composite(self.shines_bg, (11, 6))
        self._im.alpha_composite(self.rainbow_bg, (318, self._im.size[1] - 545))
        self._im.alpha_composite(self.rainbow_bottom_bg, (122, self._im.size[1] - 305))
    ...
    def whilepic(self, data: List[RaMusic], y: int = 200):
        ...
        for n, v in enumerate(data):
            if n % 20 == 0:
                ...
                x += 65
            level_index = int(v.lv)
            cover = Image.open(music_picture(v.id)).resize((55, 55))
            self._im.alpha_composite(cover, (x, y))
            if not self._diff_pg:
                self._load_image()
            self._im.alpha_composite(self._diff_pg[level_index], (x - 4, y - 4))
            self._tb.draw(x + 36, y + 3, 12, v.id, self.t_color[level_index], 'mm')
    ...
    def whilerisepic(self, data: List[RiseScore], low_score: int, isdx: bool):
        ...
        for index, _d in enumerate(data):
            ...
            self._im.alpha_composite(Image.open(music_picture(_d.song_id)).resize((80, 80)), (x + 55, y + 41))
    ...
    def draw_rise(self, sd: List[RiseScore], sd_score: int, dx: List[RiseScore], dx_score: int) -> Image.Image:
        ...
        self._sy.draw(700, height - 84, 25, '「铺面推荐不使用任何算法，仅供参考。」', self.text_color, 'mm')
        self._sy.draw(700, height - 36, 18, f'Designed by Yuri-YuzuChaN & BlueDeer233. Generated by {get_botname()} BOT', self.text_color, 'mm', 2, (255, 255, 255, 255))
        return self._im
    ...
    def draw_plan(
        ...
        self._im.alpha_composite(self.design_bg, (200, self._im.size[1] - 133))
        pagemsg = f'共计「{max}」个谱面，剩余「{len(unfinished + notstarted)}」个谱面未完成「{plan.upper()}」'
        self._sy.draw(700, self._im.size[1] - 90, 22, pagemsg, self.text_color, 'mm')
        self._sy.draw(700, self._im.size[1] - 30, 25, f'Designed by Yuri-YuzuChaN & BlueDeer233. Generated by {get_botname()} BOT', self.text_color, 'mm', 2, (255, 255, 255, 255))
        return self._im
    ...
    def draw_category(
        ...
        if category == 'completed' or category == 'unfinished':
            ...
            self._sy.draw(700, self._im.size[1] - 90, 25, pagemsg, self.text_color, 'mm')
            self._sy.draw(700, self._im.size[1] - 30, 25, f'Designed by Yuri-YuzuChaN & BlueDeer233. Generated by {get_botname()} BOT', self.text_color, 'mm', 2, (255, 255, 255, 255))
        else:
    ...
    def draw_scorelist(
        ...
        lendata = len(data)
        newdata = data[(page - 1) * 80: page * 80]
        block_height = 140 + (4 * 114)
        for n in range(0, len(newdata), 20):
            block = n // 20
            y = block_height * block
            page_data = newdata[n: n + 20]
            self._im.alpha_composite(self.title_lengthen_bg, (475, 20 + y))
            start = n + 1 + 80 * (page - 1)
            self._sy.draw(700, 67 + y, 28, f'No.{start}- No.{start + len(page_data) - 1}', self.text_color, 'mm')
            self.whiledraw(page_data, False, 140 + y)
        self._im.alpha_composite(self.design_bg, (200, self._im.size[1] - 153))
        
        pagemsg = f'「{rating}」共计「{lendata}」个成绩，'
        pagemsg += f'展示第「{(page - 1) * 80 + 1}-{80 * (page - 1) + len(newdata)}」个，'
        pagemsg += f'当前第「{page} / {end_page}」页'
        self._sy.draw(700, self._im.size[1] - 110, 25, pagemsg, self.text_color, 'mm')
        self._sy.draw(700, self._im.size[1] - 35, 25, f'Designed by Yuri-YuzuChaN & BlueDeer233. Generated by {get_botname()} BOT', self.text_color, 'mm', 2, (255, 255, 255, 255))
        return self._im
...
async def rise_score_data(
    ...
    try:
        ...
        height = 960
        image = tricolor_gradient_prism_plus(1400, height)
...
async def level_process_data(
    ...
    try:
        ...
        if category == 'default':
            ...
            completed_y = _progress_rows(clen, 5) * 109 + 140
            ulen = len(unfinished[:30])
            unfinished_y = _progress_rows(ulen, 5) * 109 + 140
            nlen = len(notplayed[:100])
            notstarted_y = _progress_rows(nlen, 20) * 65 + 140
            image = tricolor_gradient_prism_plus(1400, 150 + completed_y + unfinished_y + notstarted_y)
            dp = DrawScore(image)
            ...
            lendata = len(data)
            end_page_num = max(1, (lendata - 1) // 80 + 1)
            if page > end_page_num:
                return f'超出页数，您的成绩共计「{end_page_num}」页，请重新输入'
            topage = len(data[(page - 1) * 80: page * 80])
            plc = _progress_rows(topage, 5) * 109
            image = tricolor_gradient_prism_plus(1400, 240 + plc + 120)
            dp = DrawScore(image)
            im = dp.draw_category(category, data, page, end_page_num)
        else:
            lennotstarted = len(notplayed)
            pln = _progress_rows(lennotstarted, 20) * 65
            height = max(600, 240 + pln + 120)
            image = tricolor_gradient_prism_plus(1400, height)
            dp = DrawScore(image)
...
async def level_achievement_list_data(
    ...
    try:
        ...
        image = tricolor_gradient_prism_plus(1400, 280 + plc)
